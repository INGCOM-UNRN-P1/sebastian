---
title: "Guía de Extensión y Creación de Plugins: sebastian"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de sebastian"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-sebastian-plugins)=
# Guía de Extensión y Plugins: sebastian

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`sebastian`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-sebastian-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`sebastian` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para sebastian`.
- **Punto de Entrada Oficial**: `sebastian.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-sebastian-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para sebastian."""
    name = "custom_rule"
...
Para que `sebastian` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."sebastian.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `sebastian` la reconozca:
...
sebastian plugins list
````

---

(manual-sebastian-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `sebastian` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = sebastian.ejecutar_analisis(
...
(manual-sebastian-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `sebastian` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar sebastian
        run: pip install -e ./sebastian
        
      - name: Ejecutar Auditoría
        run: sebastian check src/ include/ --json > reporte.json
````

---

(manual-sebastian-plugins-ejercicios)=
